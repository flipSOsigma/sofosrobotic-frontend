...
export default function NewsPage () {
  ...
  useEffect(() => {
    const getSlugData = fetch('/api/news-detail/').then(res => res.json()).then(newsData => setNewsData(newsData.data[0]));
    getSlugData
  }, [])
        ...
        <div className="absolute top-0 max-w-full left-0 z-30 px-4 md:px-40 xl:px-80 text-center flex h-full justify-between items-end w-full pb-5">
          {newsData && (
            <div className="flex flex-col items-start w-full">
              <AnimatedText splitType="words" ease="power3.out" duration={0.9} text={newsData.title} className="md:text-5xl text-3xl mb-4" />
